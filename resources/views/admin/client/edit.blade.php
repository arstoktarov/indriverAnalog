@extends('admin.layouts.app', ['title' => 'Изменить клиента', 'active_index' => 'active'])
@section('content')
                            ...
                            <form action="{{route('clients.update', $client)}}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="table-responsive">
                                    <div class="form-group form-float p-t-20">
                                        <div class="form-line">
                                            <input required value="{{$client->name}}" type="text" class="form-control" id="name" name="name"/>
                                        ...
                                        <div class="form-line">
                                            <select class="form-control" name="city_id" id="city_id">
                                                @foreach($cities as $city)
                                                    <option {{$city->id === $client->city_id ? 'selected' : ''}} value="{{$city->id}}">{{$city->title}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                            <input required value="{{$client->balance}}" type="text" class="form-control" id="balance" name="balance"/>
